...
public static class Noise
{
    ...
    /// <param name="offset">Noise offset</param>
    /// <returns>Return a 2d Float array od noise</returns>
    public static float[,] GenerateNoiseMap(int mapChunk, int seed, float scale, int octaves, float persistance,
...
public static class MeshGenerator
{
    ...
    public static MeshData GenerateTerrainMesh(float[,] heightMap, float heightMultiplier,
        AnimationCurve heightInfluenceCurve, int levelOfDetail, bool useFlatShading)
    {
        ...
        for (var y = 0; y < h; y += meshSimplificationIncrement)
        {
            for (var x = 0; x < w; x += meshSimplificationIncrement)
            {
                meshData.Vertices[vertexIndex] = new Vector3(topLeftX + x,
                    heightInfluenceCurve.Evaluate(heightMap[x, y]) * heightMultiplier, topleftZ - y);
